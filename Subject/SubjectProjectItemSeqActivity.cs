...
namespace UMC.Subs.Activities
{
    class SubjectProjectItemSeqActivity : WebActivity
    {
        public override void ProcessActivity(WebRequest request, WebResponse response)
        {
            ...
            var scheduleEntity = Utility.CMS.ObjectEntity<UMC.Data.Entities.ProjectItem>();
            var projectId = Guid.Empty;
            if (ids.Length > 0)
            {
                var sub = scheduleEntity
                     .Where.Reset().And().Equal(new ProjectItem { Id = Utility.Guid(ids[0]).Value }).Entities.Single();
            ...
            {
                scheduleEntity
                    .Where.Reset().And().Equal(new ProjectItem { Id = Utility.Guid(ids[i]).Value, project_id = projectId })
                    .Entities.Update(new ProjectItem { Sequence = i });
            }
